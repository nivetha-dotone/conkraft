...
<input type="hidden" id="autoSearchParam" value="regular">
 ...
 <!-- Inline script placed immediately after dropdowns -->
    ...
    <c:if test="${UserPermission.editRights eq 1 }">
         <button type="submit" class="btn btn-default process-footer-button-cancel ng-binding" onclick="redirectToWorkmenEdit('regular')">Edit</button> 
     </c:if>
 ...
 <% if (user != null && ( !"Contractor".equals(roleName) && !"Security".equals(roleName) )) { %>
     <button type="submit" onclick="createBulkApprove(4,'regular')" class="btn btn-success process-footer-button-cancel ng-binding">
        Bulk Approve
    </button>
    <% } %>
    </div>
