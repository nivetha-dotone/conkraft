...
<input type="hidden" id="autoSearchParam" value="quick">
    ...
    <c:if test="${UserPermission.addRights eq 1 }">
        ...
        <c:if test="${UserPermission.editRights eq 1 }">
         <button type="submit" class="btn btn-default process-footer-button-cancel ng-binding" onclick="redirectToWorkmenEdit('quick')">Edit</button> 
     </c:if>
 ...
 <% if (user != null &&( !"Contractor".equals(roleName) && !"Security".equals(roleName) )) { %>
     <button type="submit" onclick="createBulkApprove(4,'quick')" class="btn btn-success process-footer-button-cancel ng-binding">
        Bulk Approve
    </button>
    <% } %>
    </div>
